...
\end{split}
\end{equation}
に対して、$\phi_{0}(x) = \sqrt{G(x)}$の場合を考える。\textcolor{blue}{\Eq{eq:231}より、無限次元の行列$\alpha\left( t_{1} - t_{2} \right)$の比対角成分が0でないため、異なる時刻の$x(t_{1})$と$x(t_{2})$が相関を持つことになる}。このとき、全系の始状態は
\textcolor{red}{
\begin{equation}
\begin{split}
\label{eq:232}
...
となる。ここで$\theta_{[0, t]}$は$G(x)$の特性関数。\textcolor{blue}{\Eq{eq:233}はすなわち、始状態$\hrho{S}(0)$から測定の周期時間$t_{\rm}$だけNon-Markovな時間発展をした後、$t = t_{\rm M}$で瞬間的に測定を行った時、測定前の目盛りが$x(t_{\rm M})$で測定後の目盛りが$x(t_{\rm M})$である時のシステムの(条件付き)状態を表している}。また、\Eq{eq:233}を、取りうる全ての測定前の目盛りについて足し合わせると、測定によって$x(t_{\rm M})$を得たときのシステムの状態が得られる。
\textcolor{red}{
\begin{equation}
\begin{split}
\label{eq:234}
...
\ref{sec:23}節において、\textcolor{blue}{ノイズ演算子は\Eq{eq:22}より(回転波近似を適用した場合の)測定器の測定量であると解釈ができる}。そのため、$z(t)$を測定器の目盛りに対応した量と捉え
\textcolor{red}{
\begin{equation}
\begin{split}
\label{eq:235}
...
となる。\textcolor{red}{\Eq{eq:239}は\Eq{eq:228}の解である}。\textcolor{red}{したがって、\Eq{eq:228}は
\begin{equation}
\begin{split}
\label{eq:240}
